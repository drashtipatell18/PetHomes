...
const getData = async () => {
  ...
  if (serv) {
    if (serv == 'All') {
      servData = servicesdata;
    } else {
      servData = servicesdata.filter(v => v.name.toLowerCase().includes(serv.toLowerCase()));
    }
  }
  filterData = servicesdata;

  listSubcategory(filterData);
  createCards(servData);
}
...
const createCards = (data) => {
  ...
  document.getElementById("displayservices").innerHTML = print;
...
const listSubcategory = (data) => {
  // console.log(data);
  const uniqueSubcategories = [...new Set(data.map((v) => v.name))];
  let print = `<li>
  ...
  uniqueSubcategories.slice(0, 6).forEach((name, i) => {
    print += `<li>
                    <label for="breed-${i + 1}" class="playlist-name m_radio1">
                      <input id="breed-${i + 1}" type="radio" name="a_breed" value="${name}">
                      ${name}
                    </label>
                  </li>`;
  });
  ...
  uniqueSubcategories.slice(6).forEach((name, i) => {
    print += `<li class="more">
                    <label for="breed-${i + 7}" class="playlist-name m_radio1">
                      <input id="breed-${i + 7}" type="radio" name="a_breed" value="${name}">
                      ${name}
                    </label>
                  </li>`;
  });
...
const handleSubcategory = (event) => {
  ...
  if (selectedValue == "All") {
    ...
    // finaldata = filterData;
    finaldata = filterData.filter((v) => v.name == selectedValue);
  }
...
const handlesearch = (event) => {
  ...
  let filt = filterData.filter(item => item.name.toLowerCase().includes(searchText));
  createCards(filt);
}
...
const handledeatails = (id) => {
  window.open(`dog_view.html?page=services&id=${id}`, "_self");
}
...
const handlesearchall = (e) => {
  ...
  const searchData = [...petdata, ...accdata, ...servicesdata];
